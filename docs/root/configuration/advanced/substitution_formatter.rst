...
``%REQ_ALL_HEADERS%``
  HTTP
    ...
    and the header value becomes the corresponding string value. Repeated headers with the
    same name are comma-joined per RFC 7230. Pseudo-headers (``:method``, ``:path``,
    ``:authority``) are included. When no request headers are available, ``"-"`` appears
    in the access log (or ``null`` in JSON format).
