...
Protobuf::Struct AllHeadersFormatter::buildHeadersStruct(const Http::HeaderMap& headers) const {
  ...
  headers.iterate([&](const Http::HeaderEntry& entry) -> Http::HeaderMap::Iterate {
    ...
    if (it != fields->end()) {
      ...
      (*fields)[key].set_string_value(std::move(val));
    }
    return Http::HeaderMap::Iterate::Continue;
  });
...
AllHeadersCommandParser::parse(absl::string_view command, absl::string_view,
                               absl::optional<size_t>) const {
  if (command == "REQ_ALL_HEADERS") {
    return std::make_unique<AllHeadersFormatter>(AllHeadersType::Request, max_value_bytes_,
                                                 exclude_headers_);
  }
  if (command == "RESP_ALL_HEADERS") {
    return std::make_unique<AllHeadersFormatter>(AllHeadersType::Response, max_value_bytes_,
                                                 exclude_headers_);
  }
  return nullptr;
}
